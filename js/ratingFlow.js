...
function getAllLessonsFromDB(lessonList) {
  for (key in lessonList) {//extract all lesson data
    ...
    //check if we want to display the lesson

    if (isPassLesson(lesson.date, lesson.time) && lessonList[key].rated != "true")
  ...
  allLessons.forEach(element => {//write lessons to html
    ...
    $("#lesson_data_table").append(
      '<tr>' +
            ...
            '<td><button id="" type="button" class="btn  saveClicked btn-primary" id_in_array="' + counter++ +
             '" data_t_uid="' + element.teacher_uid + '" data_l_uid="' + element.lesson_id + '">Save</button></td>' +
            '</tr>'
    );
  });
  initEvents();
...
function initEvents() {
  $(".saveClicked").on("click", function () {
    ...
    firebase.database().ref("/user/client/" + firebase_init.get_user().uid +
    "/my_lessons_list/" + lesson_id).update(updates_class_list);
